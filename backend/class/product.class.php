...
class Product
{
	...
	*/
	...
	public function addProduct($data){
		...
		if (isset($data['image'])) {
			$this->db->query("UPDATE " . DB_PREFIX . "product SET image = '" . $data['image'] . "' WHERE product_id = '" . (int)$product_id . "'") or die($sql);
		}
			...
			foreach ($data['product_option'] as $product_option) {
				if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
					if (isset($product_option['product_option_value'])) {
						$this->db->query("INSERT INTO " . DB_PREFIX . "product_option SET product_id = '" . (int)$product_id . "', option_id = '" . (int)$product_option['option_id'] . "', required = '" . (int)$product_option['required'] . "'") or die('fgjjj');

						$product_option_id = $this->db->getLastId();

						foreach ($product_option['product_option_value'] as $product_option_value) {
							$this->db->query("INSERT INTO " . DB_PREFIX . "product_option_value SET product_option_id = '" . (int)$product_option_id . "', product_id = '" . (int)$product_id . "', option_id = '" . (int)$product_option['option_id'] . "', option_value_id = '" . (int)$product_option_value['option_value_id'] . "', quantity = '" . (int)$product_option_value['quantity'] . "', subtract = '" . (int)$product_option_value['subtract'] . "', price = '" . (float)$product_option_value['price'] . "', price_prefix = '" . $this->escape($product_option_value['price_prefix']) . "', points = '" . (int)$product_option_value['points'] . "', points_prefix = '" . $this->escape($product_option_value['points_prefix']) . "', weight = '" . (float)$product_option_value['weight'] . "', weight_prefix = '" . $this->escape($product_option_value['weight_prefix']) . "'") or die('uywtrgsdf');
						}
					}
				} else {
					$this->db->query("INSERT INTO " . DB_PREFIX . "product_option SET product_id = '" . (int)$product_id . "', option_id = '" . (int)$product_option['option_id'] . "', value = '" . $product_option['value'] . "', required = '" . (int)$product_option['required'] . "'") or die('dhgjoiyherpt');
				}
			}
		}
		...
		if (isset($data['product_image'])) {
			foreach ($data['product_image'] as $product_image) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_image SET product_id = '" . (int)$product_id . "', image = '" . $this->escape($product_image['image']) . "', sort_order = '" . (int)$product_image['sort_order'] . "'") or die('iugvogbq');
			}
		}

		if (isset($data['product_download'])) {
			foreach ($data['product_download'] as $download_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_to_download SET product_id = '" . (int)$product_id . "', download_id = '" . (int)$download_id . "'") or die('iughasvougadfovhoadf0ihn');
			}
		}
		...
		if (isset($data['product_filter'])) {
			foreach ($data['product_filter'] as $filter_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_filter SET product_id = '" . (int)$product_id . "', filter_id = '" . (int)$filter_id . "'") or die('vubaven');
			}
		}

		if (isset($data['product_related'])) {
			foreach ($data['product_related'] as $related_id) {
				$this->db->query("DELETE FROM " . DB_PREFIX . "product_related WHERE product_id = '" . (int)$product_id . "' AND related_id = '" . (int)$related_id . "'") or die('ufg40734');
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_related SET product_id = '" . (int)$product_id . "', related_id = '" . (int)$related_id . "'") or die('98fq3tgvub');
				$this->db->query("DELETE FROM " . DB_PREFIX . "product_related WHERE product_id = '" . (int)$related_id . "' AND related_id = '" . (int)$product_id . "'") or die('yfvgcp3qb');
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_related SET product_id = '" . (int)$related_id . "', related_id = '" . (int)$product_id . "'") or die('bv2ergw	gvawr');
			}
		}
		...
		if (isset($data['product_shop'])) {
			foreach ($data['product_shop'] as $index => $shop_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_to_shop SET product_id = '" . (int)$product_id . "', shop_id = '" . (int)$shop_id . "'");
			}
		}

		if (isset($data['product_size'])) {
			foreach ($data['product_size'] as $size_id => $group_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_to_size SET product_id = '" . (int)$product_id . "', size_id = '" . (int)$size_id . "'");
			}
		}
		...
		if (isset($data['keyword'])) {
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'product_id=" . (int)$product_id . "', keyword = '" . $this->escape($data['keyword']) . "'");
		}
	...
	public function editProduct($product_id, $data) {
		...
		if (isset($data['image'])) {
			$this->db->query("UPDATE " . DB_PREFIX . "product SET image = '" . $this->escape($data['image']) . "' WHERE product_id = '" . (int)$product_id . "'");
		}
		...
		//Если есть разница по ценам
		$this->db->query("DELETE FROM " . DB_PREFIX . "product_attribute WHERE product_id = '" . (int)$product_id . "' AND attribute_id='580'");
		if (isset($data['oldprice'])) {
			if($data['oldprice'] > 0 AND $data['oldprice'] > $data['price']) {
				$data['product_attribute'][580]['attribute_id'] = 580;
				$data['product_attribute'][580]['name'] = '1';
		...
		if (isset($data['product_option'])) {
			foreach ($data['product_option'] as $product_option) {
				if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
					...
					$this->db->query("INSERT INTO " . DB_PREFIX . "product_option SET product_option_id = '" . (int)$product_option['product_option_id'] . "', product_id = '" . (int)$product_id . "', option_id = '" . (int)$product_option['option_id'] . "', value = '" . $this->escape($product_option['value']) . "', required = '" . (int)$product_option['required'] . "'");
				}
			}
		}
		...
		if (isset($data['product_size'])) {
			foreach ($data['product_size'] as $size_id => $group_id) {
				$sql = "INSERT INTO " . DB_PREFIX . "product_to_size SET product_id = '" . (int)$product_id . "', size_id = '" . (int)$size_id . "'";
				$this->db->query($sql) or die($sql);
			}
		}
		...
		if (isset($data['product_shop'])) {
			foreach ($data['product_shop'] as $index => $shop_id) {
				$this->db->query("DELETE FROM " . DB_PREFIX . "product_to_shop WHERE product_id = '" . (int)$product_id . "' AND shop_id = '" . (int)$shop_id . "'");
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_to_shop SET product_id = '" . (int)$product_id . "', shop_id = '" . (int)$shop_id . "'");
			}
		}
		...
		if (isset($data['product_image'])) {
			foreach ($data['product_image'] as $product_image) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_image SET product_id = '" . (int)$product_id . "', image = '" . $this->escape($product_image['image']) . "', sort_order = '" . (int)$product_image['sort_order'] . "'");
			}
		}
	...
	public function getProducts($data = array()) {
		$sql = "SELECT * FROM " . DB_PREFIX . "product p
				...
				LEFT JOIN " . DB_PREFIX . "product_to_shop p2s ON (p.product_id = p2s.product_id)
	...
	public function getProduct($product_id) {
		$sql = "SELECT DISTINCT *, pd.name AS name, p.image, m.name AS manufacturer,
					...
					(SELECT sh.shop_id FROM " . DB_PREFIX . "product_to_shop sh
