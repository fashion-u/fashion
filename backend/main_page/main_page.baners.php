...
if(isset($_POST['key']) AND $_POST['key'] == 'add'){
	...
	echo "<H2>Добавлено! Подождите, идет сохранение.</H2><SCRIPT>\n\nvar i = setTimeout(\"window.location.href='/".TMP_DIR."backend/index.php?route=main_page/main_page.index.php&modul=main_page.baners.php'\", 500);\n</SCRIPT>";
}
?>
<br>
<h1>Загрузка банера для главной страницы. Размер 563 х 165 px!</h1>
    ...
    <div class="table_body">
		...
		<tr><form method=post>
			...
			<td>
			  ...
			  <!--br>Текст для кнопки</b> : -->
				<input type="hidden" name="title" class="brand" data-id="0" style="width:400px;" value="" placeholder="Текст для кнопки">
			</td>
...
<?php while($value = $r->fetch_assoc()){ ?>
 <tr id="<?php echo $value['baner_id']; ?>">
	  ...
	  <td><img src="<?php echo $uploaddir.$value['baner_pic']; ?>" width="250" height="73">
	  </td>
      ...
      <td>
		  ...
		  <!--br>Текст для кнопки</b> : --><input type="hidden" id="title<?php echo $value['baner_id'];?>" class="brand edit" data-id="<?php echo $value['baner_id']; ?>" style="width:400px;" value="<?php echo $value['baner_title']; ?>">
	  </td>
